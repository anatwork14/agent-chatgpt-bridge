...
test("SessionManager isolated execution", async () => {
  ...
  const manager = new SessionManager(sessionStore, messageStore, turnStore, { fake: provider });
...
test("SessionManager serialization", async () => {
  ...
  const manager = new SessionManager(sessionStore, messageStore, turnStore, { slow: slowProvider as any });
...
test("SessionManager cancel and close", async () => {
  ...
  await manager.cancel(s.id, "turn_1");
...
test("SessionManager preserves a thrown BridgeError code in turn persistence", async () => {
  ...
  const manager = new SessionManager(sessionStore, messageStore, turnStore, { limited: provider });
