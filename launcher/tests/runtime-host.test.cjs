...
test("failed terminal migration restores removed launchd ownership before verifying the old runtime", async () => {
  ...
  const configPath = path.join(coreHome, "config.json");
  const daemonPlist = path.join(launchAgentsDir, "io.github.agent-chatgpt-bridge.daemon.plist");
  const tunnelPlist = path.join(launchAgentsDir, "io.github.agent-chatgpt-bridge.tunnel.plist");
  const oldConfig = {
    mode: "full",
